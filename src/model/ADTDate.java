...
public class ADTDate implements Comparable<ADTDate>, IDate{
    private int theDay;
    ...
    private boolean dateMatch(ADTDate aDate){
        if(this.theYear != aDate.theYear){
            return false;
        }
        else if(this.theMonth != aDate.theMonth){
            return false;
        }
        else if(this.theDay != aDate.theDay){
            return false;
        }
        else return true;
    }
     ...
     *
     * @param theCompDate requires an object of ADTDate
     * @return an integer value 0 - equal, 1 - not equal
     */
    @Override
    public int compareTo(ADTDate theCompDate){
        if(this.dateMatch(theCompDate))
            return 0;
        else
            return 1;
    }
